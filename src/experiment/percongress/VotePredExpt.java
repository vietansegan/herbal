...
public class VotePredExpt extends AbstractExperiment<Congress> {
    ...
    private void setLegislatorPropertyValues(AuthorVoteTextDataset phaseData, String lid) {
        ...
        phaseData.setAuthorProperty(lid, "party", leg.getParty());
    ...
    private void summarizeAuthorScores(File estimateFolder) {
        ...
        ArrayList<String> authorVocab = debateVoteData.getAuthorVocab();
        int A = authorVocab.size();

        ArrayList<String> models = new ArrayList<>();
        ArrayList<double[]> authorEstimates = new ArrayList<>();

        // average
        models.add("average");
        double[] avgEsts = new double[A];
        for (int aa = 0; aa < A; aa++) {
            ...
            avgEsts[aa] = (double) withCount / totalCount;
        }
        authorEstimates.add(avgEsts);

        // ideal point
        IdealPoint ip = new IdealPoint("ideal-point");
        models.add(ip.getBasename());
        ...
        authorEstimates.add(mmNorm.normalize(bip.getUs()));
        ...
        try {
            ...
            writer.write("LegislatorID\tLegislatorName\tParty");
            ...
            for (int aa = 0; aa < debateVoteData.getAuthorVocab().size(); aa++) {
                ...
                String authorName = debateVoteData.getAuthorProperty(authorId, "name");
                String authorParty = debateVoteData.getAuthorProperty(authorId, "party");
                writer.write(authorId + "\t" + authorName + "\t" + authorParty);
                for (double[] est : authorEstimates) {
                    writer.write("\t" + est[aa]);
                }
                writer.write("\n");
            }
    ...
    protected void runLDA(File estimateFolder) {
        ...
        // debug
        ArrayList<Integer> docIndices = new ArrayList<>();
        for (int ii = 0; ii < 2000; ii++) {
            docIndices.add(ii);
        }

        if (cmd.hasOption("train")) {
            sampler.train(debateVoteData.getWords(), docIndices);
    ...
    protected void runIdealPoint(File outputFolder) {
        ...
        if (cmd.hasOption("train")) {
            ...
            outputAuthorScore(new File(predFolder, AuthorScoreFile),
                    debateVoteData.getAuthorVocab(), this.trainAuthorIndices,
                    pred.getUs(),
    ...
    protected void runSLDAIdealPoint(File outputFolder) {
        ...
        double epsilon = CLIUtils.getDoubleArgument(cmd, "epsilon", 0.01);
        ...
        sampler.configure(outputFolder.getAbsolutePath(),
                ...
                alpha, beta, rho, mu, sigma, epsilon,
                initState, paramOpt,
                burn_in, max_iters, sample_lag, report_interval);
        ...
        if (cmd.hasOption("train")) {
            ...
            outputAuthorScore(new File(samplerFolder, AuthorScoreFile),
                    debateVoteData.getAuthorVocab(), trainAuthorIndices,
                    sampler.getUs(),
                    debateVoteData.getAuthorTable());
            outputVoteScores(new File(samplerFolder, VoteScoreFile),
                    debateVoteData.getVoteVocab(), trainBillIndices,
    ...
    protected void outputAuthorScore(File outputFile,
            ArrayList<String> authorVocab, ArrayList<Integer> authorIndices,
            double[] authorScores,
            HashMap<String, Author> authorTable) {
        this.trainAuthorIndices = authorIndices;
        ...
        if (this.trainAuthorIndices.size() != authorScores.length) {
            throw new RuntimeException("Mismatch. " + this.trainAuthorIndices.size()
                    + " vs. " + authorScores.length);
        }

        System.out.println("Outputing to " + outputFile);
        try {
            BufferedWriter writer = IOUtils.getBufferedWriter(outputFile);
            writer.write("Index\tID\tScore\tNumWithVotes\tNumAgainstVotes"
                    + "\tFreedomWorksID\tName\tParty\tNominateScore\n");
            for (int ii = 0; ii < this.trainAuthorIndices.size(); ii++) {
